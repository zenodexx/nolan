const fs = require('fs');
const votacao = JSON.parse(fs.readFileSync('./BANK-NOLAN/func/votacao/voting.json'))
...
fs.writeFileSync(`./BANK-NOLAN/func/votacao/voting.json`, JSON.stringify(votacao))
fs.unlinkSync(`./BANK-NOLAN/func/votacao/votos/${IDGRUPO}.json`)
fs.unlinkSync(`./BANK-NOLAN/func/votacao/p_votos/${IDGRUPO}.json`)
}

const AdicionarVoto = async(from, R1, R2, R3, R4, mention) => {
votacao.push(IDGRUPO)
fs.writeFileSync(`./BANK-NOLAN/func/votacao/p_votos/${IDGRUPO}.json`, `[]`)
fs.writeFileSync(`./BANK-NOLAN/func/votacao/votos/${IDGRUPO}.json`, `[]`)
fs.writeFileSync('./BANK-NOLAN/func/votacao/voting.json', JSON.stringify(votacao))
await sleep(2000)
let _votos = JSON.parse(fs.readFileSync(`./BANK-NOLAN/func/votacao/votos/${IDGRUPO}.json`))
_votos.push({razao: R1 ? R1 : '-', votos: R2.trim() + '@s.whatsapp.net', votos2: R3.trim() + '@s.whatsapp.net', duracao: R4})
fs.writeFileSync(`./BANK-NOLAN/func/votacao/votos/${IDGRUPO}.json`, JSON.stringify(_votos))
setTimeout(async function() {
let voto = JSON.parse(fs.readFileSync(`./BANK-NOLAN/func/votacao/p_votos/${IDGRUPO}.json`))
...
fs.writeFileSync(`./BANK-NOLAN/func/votacao/votos/${from}.json`)
fs.writeFileSync(`./BANK-NOLAN/func/votacao/p_votos/${from}.json`)
fs.writeFileSync("./BANK-NOLAN/func/votacao/voting.json", JSON.stringify(votacao))
}, R4*60*1000)
}
